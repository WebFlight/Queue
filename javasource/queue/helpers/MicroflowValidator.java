...
public class MicroflowValidator {
	...
	public boolean hasInputParameterOfTypeObject(Map<String, IDataType> inputParameters) {
		return inputParameters.values().stream().filter(p -> p.getType() == DataTypeEnum.Object).anyMatch(p -> Core.getMetaObject(p.getObjectType()).isSubClassOf(Core.getMetaObject("Queue.Job")));
	}
