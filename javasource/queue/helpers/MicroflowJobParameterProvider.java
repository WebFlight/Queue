...
public class MicroflowJobParameterProvider {
	...
	public String getJobParameterName(String microflowName) {
		...
		String parameterName = inputParameters.entrySet().stream().filter(p -> microflowRepository.getMetaObject(p.getValue().getObjectType()).isSubClassOf(microflowRepository.getMetaObject("Queue.Job"))).findFirst().get().getKey();
