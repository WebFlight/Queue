...
2. Use the documents in the _USE_ME folder or the actions in the Queue section of the toolbox:
	...
	- The AddJobToQueue action will add a new job in a queue that has been initialized before. Make sure the microflow to be executed as an input parameter with data type Object and entity Queue.Job. If you need to use a specialization of a Job entity, cast the object in the microflow to the specialization type. If you would like to enter the microflow name as a String, use the AddJobToQueueTextToMicroflow action in the private folder. This action extracts the name from the MicroflowName attribute in the Job.
